...
抓 MP 中本 14 元素空間、所有含磁性主族的 2-/3-元素系統的 **261 個鐵磁(FM)金屬間
...
用 261 個 MP FM 化合物訓練 GBR（5-fold 交叉驗證）：

| 模型 | CV R² | CV MAE |
|---|---|---|
| 合成 Br | +0.006 | 0.52 T |
| **真實資料 GBR** | **+0.601** | **0.26 T** |

→ **Br 變成可預測**：R² 由 ~0 升到 0.60、MAE 0.52→0.26 T。R² 略低於 Tc 的 0.78，但
方向與機制完全一致、且經交叉驗證。
...
| **FM-only（335 系統，採用）** | **261** | **0.601** | **0.26 T** |
...
- R²=0.60（中等）：261 點的金屬 FM 磁化資料仍有限；放寬 chemsys 已從 0.56 提到 0.60。
...
```bash
python scripts/build_mp_magnetization_dataset.py   # 需 MP key；抓 261 化合物
python scripts/train_br_mp_baseline.py             # → 合成 0.006 vs 真實 0.601
```
