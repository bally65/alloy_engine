...
### 證明：真實資料把 R² 從 −0.17 救回 0.88（已實跑）

`scripts/train_surrogate_nemad_baseline.py` 直接用真實 NEMAD Tc 訓練
（1014 筆，810/102/102 切分，36 維 Oliynyk 特徵）：

| 模型 | 測試 R² | MAE |
|---|---|---|
| 合成代理（評估於真實）| −0.17 | 274°C |
| **真實 NEMAD 訓練（baseline）** | **0.88** | **81°C** |

→ **資料可用、方案可運行已證實**；且與 NEMAD 論文報告的 R²≈0.86 一致。
（窄帶子集 e1/e2/e3 R² 為負是 n=5–11 太小所致，整體 0.88 為準。）

### D3 也通了：MP 磁化已可取用，並量出 Br 偏差（已實跑）

MP API key 已驗證可用、API 可連線。`scripts/mp_magnetization_eval.py`（raw
HTTPS，免裝 mp-api）對 8 個已知鐵磁體比較「合成 Br vs MP 真實 DFT 磁化」：

| 指標 | 值 |
|---|---|
| MAE | 0.53 T |
| **系統性偏差 bias** | **−0.50 T（合成 Br 系統性低估）** |

逐材料：FeCo/Co/Co3Fe 接近（±0.1T），但 Fe(−1.37)、Gd(−1.09)、Fe3Si/Ni3Fe
(−0.6) 低估明顯。**這證實了 README 早記的「Br 低估」限制，並量化了它**——
代表本引擎的 delta_M 與整機功率密度是**系統性偏保守（低估）**。

> 注意：MP 為 0K DFT 飽和極化，合成 Br 含室溫/合成校準，故部分負偏差屬預期
> （室溫 Br < 0K 飽和）。直接拿 MP 值「校正」需先區分 0K 飽和 vs 工作溫度
> 磁化，否則會把 delta_M 推高到不對的方向。

### 已謹慎處理：溫度修正後重做同溫對標（已實跑）

`alloy_engine/thermomagnetic/magnetization_correction.py` 用**平均場
Brillouin 約化磁化** m(T/Tc) 把 MP 的 0K 飽和值降到工作溫度（300K），
與合成 Br 同溫可比（**正確方向：把 0K 往下拉，而非把合成往上推**）：

| 對標基準 | MAE | bias | 結論 |
|---|---|---|---|
| 對 MP 0K（raw） | 0.53 T | **−0.50 T** | 0K-vs-室溫 蘋果比橘子 |
| 對 MP@300K（溫度修正） | 0.59 T | **−0.15 T** | 同溫可比；bias 大幅縮小 |

→ **原 −0.50T 偏差多源於「0K 飽和 vs 室溫」的溫度差，而非模型系統性低估**。
Gd 是揭露性的離群點：其 Tc=293K 恰在室溫，故 300K 時已順磁（M→0），裝置
須在 Tc 以下運作——這也解釋了為何近室溫 MCE 材料不能用固定 300K 的「Br」評斷。
...
### 附：如何取得 Materials Project API key（只 D3 需要，2 分鐘）

1. 到 https://next-gen.materialsproject.org → 右上 **Log in**（可用 Google /
   GitHub / ORCID，免費）。
2. 登入後進 **Dashboard → API**（或 https://next-gen.materialsproject.org/api）
   → 複製你的 **API Key**（一串 32 字元）。
3. 給我的方式（**別貼在公開聊天/PR**）：建議存成 **GitHub Secret**
   `MP_API_KEY`（Repo → Settings → Secrets → Actions），GHA workflow 即可
   安全取用；或本機設環境變數 `export MP_API_KEY=...`。

> 我無法自己註冊 MP 帳號（需人工 email/OAuth 驗證），這是 D3 唯一需要你
> 動手的一步。在那之前，真實 **Tc** 已可用（上方 R²=0.88），只差真實
> **磁化（Br→delta_M）** 待這把 key 解鎖。
